...
def main():
    ...
    # 2. Train-Test Split (with stratification on priority)
    ...
    print("Step 4: Training Logistic Regression priority classifier...")
    ...
    print("\n" + "="*80)
    print("VISUALIZING THE PRIORITY CONFUSION MATRIX")
    print("="*80)
    
    classes = sorted(df['priority'].unique())
    ...
    print("PRIORITY CLASSIFICATION METRICS (Precision, Recall, F1-Score)")
    print("="*80)
    print(classification_report(y_test, y_pred, target_names=classes))
    print("="*80)

    # 8. Serialize and Save Priority Model
    models_dir = "models"
    model_path = os.path.join(models_dir, "priority_classifier.joblib")
    
    print("Step 8: Saving Priority Model to disk...")
    joblib.dump(model, model_path)
    print(f"Priority classifier successfully saved to: {model_path}")
